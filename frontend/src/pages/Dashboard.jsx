    ...
    <div className="flex flex-col min-h-full bg-[#111] text-white">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-10 space-y-6 scrollbar-hide">
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-4xl font-black text-[#c8f31d]">71.0 <span className="text-lg text-zinc-400">kg</span></p>
            </div>
            <div className="text-right">
              <p className="text-xs text-zinc-500 mb-1">Mục tiêu</p>
              <p className="text-2xl font-black text-white">68.0 <span className="text-sm text-zinc-400">kg</span></p>
            </div>
          </div>
          ...
          <div className="relative h-48 mb-4">
            <svg className="w-full h-full" viewBox="0 0 280 120" preserveAspectRatio="none">
              ...
              {/* Area */}
              <polyline
                points={data.map((d, i) => `${i * 40 + 10},${((maxW - d.value) / (maxW - minW)) * 110 + 5}`).join(' ')}
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Đã giảm', value: '1.5 kg', color: '#22c55e' },
            { label: 'Còn lại', value: '3.0 kg', color: '#f97316' },
  ...
  return (
    <div className="flex flex-col pb-6 text-white bg-[#111]">
      ...
      <div className="px-6 -mt-4 relative z-20 space-y-5 pt-8">
        ...
        {/* Chọn mục tiêu */}
        <div>
          <h2 className="text-lg font-bold mb-4 text-zinc-100">Chọn mục tiêu</h2>
          <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2" style={{ scrollbarWidth: 'none' }}>
            {['Giảm cân', 'Tăng cân', 'Thể hình'].map((g, i) => (
              <button key={i} className={`whitespace-nowrap px-5 py-2 rounded-lg text-sm font-medium ${i === 1 ? 'bg-[#c8f31d] text-black font-bold' : 'border border-zinc-700 text-zinc-300'}`}>{g}</button>
            ))}
          </div>
        </div>
