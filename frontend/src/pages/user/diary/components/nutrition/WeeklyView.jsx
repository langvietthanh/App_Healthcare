...
const ChartSection = ({ totalWeekKcal, totalWeekGoal, weeklyData, radius, circumference, offset }) => (
  // GIAO DIỆN CHART SECTION
  <div className="flex items-center gap-6 mb-8 mt-4">
    {/* VÒNG TRÒN TỔNG CALO */}
    <div className="flex-none flex flex-col items-center justify-center">
      <div className="relative w-[140px] h-[140px] flex items-center justify-center">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 160 160">
          <circle cx="80" cy="80" r={radius} stroke="#3f3f46" strokeWidth="12" fill="transparent" />
          <circle cx="80" cy="80" r={radius} stroke="#f97316" strokeWidth="12" fill="transparent"
            strokeDasharray={circumference} strokeDashoffset={offset}
            strokeLinecap="round" className="transition-all duration-1000" />
        </svg>
        <div className="absolute flex flex-col items-center">
          <span className="text-4xl font-black text-[#f97316] leading-none">{Math.round(totalWeekKcal)}</span>
          <div className="w-10 h-[2px] bg-white/30 my-1.5"></div>
          <span className="text-sm font-bold text-zinc-400">{Math.round(totalWeekGoal)}</span>
        </div>
      </div>
    </div>
