...
const FoodList = ({ activeTab, setIsCreatingFood, handleSelectFood, listFoods, loading }) => (
  // GIAO DIỆN FOOD LIST
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-zinc-800 rounded-xl overflow-hidden border border-zinc-700 flex items-center justify-center shrink-0">
              {food.image && (food.image.startsWith('http') || food.image.startsWith('/') || food.image.includes('.')) ? (
                <img src={food.image.startsWith('/') ? `http://localhost:3000${food.image}` : food.image} alt={food.name} className="w-full h-full object-cover" />
              ) : (
