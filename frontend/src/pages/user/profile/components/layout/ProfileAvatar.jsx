...
const ProfileAvatar = ({ user }) => {
  return (
    ...
    <div className="flex flex-col items-center mt-2 mb-12">
      ...
      <h2 className="text-3xl font-semibold mb-2">{user?.username || 'Đang tải...'}</h2>
      <p className="text-zinc-400 font-medium text-sm">
        {user?.role === 'admin' ? 'Quản trị viên' : user?.role === 'user' ? 'Thành viên' : user?.role}
      </p>
    </div>
  );
};
