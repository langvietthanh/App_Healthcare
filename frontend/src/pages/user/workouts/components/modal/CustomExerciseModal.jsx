...
const ModalHeader = ({ onClose, isEdit }) => (
  <div className="flex items-center justify-between mb-6">
    <h2 className="text-xl font-black text-white">{isEdit ? 'Sửa bài tập cá nhân' : 'Tạo bài tập cá nhân'}</h2>
...
const ImageSection = ({ form, setForm, fileInputRef }) => {
...
const BasicInfoSection = ({ form, setForm, CATEGORIES }) => (
...
const MusclesSection = ({ form, setForm, MUSCLES }) => {
...
const InstructionsSection = ({ form, setForm }) => {
...
const ActionButtons = ({ onClose, handleSave, loading, isEdit }) => (
  <div className="flex gap-3 mt-8">
    ...
    >
      {loading ? 'Đang lưu...' : <><Check size={20} /> {isEdit ? 'Lưu thay đổi' : 'Tạo bài tập'}</>}
    </button>
  </div>
);

const CustomExerciseModal = ({ onClose, onSuccess, initialData = null }) => {
  const [form, setForm] = useState(() => {
    if (initialData) {
      return {
        name: initialData.name || '',
        category: initialData.type || CATEGORIES[0],
        targetMuscles: initialData.muscles ? initialData.muscles.map(m => ({
          muscle: m, // It's already in Vietnamese
          rating: initialData.rating || 5
        })) : [],
        description: initialData.description || '',
        instructions: initialData.instructions?.length ? initialData.instructions : [''],
        image: initialData.img && initialData.img !== '🏋️' && initialData.img !== '🏃‍♂️' ? initialData.img : '',
        imageFile: null
      };
    }
    return {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (initialData && initialData.id) {
        await axiosClient.put(`/exercises/${initialData.id}?type=exercise`, formData, {
  ...
  return (
    // GIAO DIỆN CUSTOM EXERCISE MODAL
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-hide animate-in zoom-in-95 duration-200 text-left text-white">
        <ModalHeader onClose={onClose} isEdit={!!initialData} />

        <div className="space-y-6">
          <ImageSection form={form} setForm={setForm} fileInputRef={fileInputRef} />
          <BasicInfoSection form={form} setForm={setForm} CATEGORIES={CATEGORIES} />
          <MusclesSection form={form} setForm={setForm} MUSCLES={MUSCLES} />
          <DescriptionSection form={form} setForm={setForm} />
          <InstructionsSection form={form} setForm={setForm} />
        </div>

        <ActionButtons onClose={onClose} handleSave={handleSave} loading={loading} isEdit={!!initialData} />
      </div>
    </div>
  );
};
