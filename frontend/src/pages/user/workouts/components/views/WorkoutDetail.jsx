...
import { ArrowLeft, Clock, ChevronDown, ChevronUp, Edit3, Trash2 } from 'lucide-react';
import { useWorkout } from '../../../../../providers/user/workout';
import axiosClient from '../../../../../config/axiosClient';
import CustomExerciseModal from '../modal/CustomExerciseModal';

const DetailHeader = ({ setView }) => (
  // GIAO DIỆN DETAIL HEADER
...
const ExerciseInfo = ({ selectedExercise, onEdit, onDelete }) => {
  const [showInstructions, setShowInstructions] = useState(false);

  return (
    // GIAO DIỆN EXERCISE INFO
    <div className="bg-zinc-900 rounded-3xl p-6 mb-8 border border-zinc-800 flex flex-col gap-4 shadow-lg">
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 relative">
          {selectedExercise?.creatorId && (
            <div className="absolute top-0 right-0 flex gap-2">
              <button onClick={onEdit} className="p-1 text-zinc-400 hover:text-[#c8f31d] transition-colors" title="Sửa bài tập">
                <Edit3 size={18} />
              </button>
              <button onClick={onDelete} className="p-1 text-zinc-400 hover:text-red-500 transition-colors" title="Xóa bài tập">
                <Trash2 size={18} />
              </button>
            </div>
          )}
          <h3 className="text-xl font-black text-white mb-2 leading-tight pr-14">{selectedExercise?.name}</h3>
...
const WorkoutDetail = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosClient.delete(`/exercises/${selectedExercise.id}`);
      fetchExercisesFromBackend();
      setWorkoutView('search');
    } catch (error) {
      console.error(error);
      alert(error.response?.data?.message || 'Có lỗi xảy ra khi xóa bài tập');
    }
  };
  ...
  const handleEditSuccess = async () => {
    ...
    setWorkoutView('search'); // Quay lại màn hình search sau khi sửa thành công
  };

  return (
    // GIAO DIỆN WORKOUT DETAIL
