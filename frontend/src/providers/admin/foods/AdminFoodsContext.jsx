...
const AdminFoodsProvider = ({ children }) => {
    ...
    const setOriginFilter = (val) => dispatch({ type: 'SET_ORIGIN_FILTER', payload: val });
    const setTimeFilter = (val) => dispatch({ type: 'SET_TIME_FILTER', payload: val });
    const toggleFilter = () => dispatch({ type: 'TOGGLE_FILTER' });
    const setShowFilter = (val) => dispatch({ type: 'SET_SHOW_FILTER', payload: val });
    const setDeleteId = (val) => dispatch({ type: 'SET_DELETE_ID', payload: val });
    ...
    const handleTogglePublic = async (id, currentStatus) => {
        try {
            await axiosClient.patch(`/foods/${id}`, { isPublic: !currentStatus });
            fetchFoods();
        } catch (err) {
            console.error('Error toggling public status:', err);
            alert(err.response?.data?.message || 'Có lỗi xảy ra khi đổi trạng thái');
        }
    };
